...
const optionsSchema = z.object({
  options: z
    .array(z.string().max(100, { message: "identifiant d'option trop long (maximum 100 caractères)" }))
    .max(50, { message: "trop d'options sélectionnées (maximum 50)" }),
});
...
const paiementSchema = z.object({
  creneauId: z
    .string()
    .max(100, { message: "l'identifiant de créneau est trop long (maximum 100 caractères)" })
    .nullable()
...
export const devisSchema = z.object({
  ...
  besoin: z
    ...
    .max(2000, { message: "la description du besoin est trop longue (maximum 2000 caractères)" }),
});

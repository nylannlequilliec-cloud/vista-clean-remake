...
describe("devisSchema length constraints", () => {
  it("rejette les prénoms dépassant 100 caractères", () => {
    const validData = {
      prenom: "A".repeat(100),
      telephone: "0612345678",
      besoin: "Besoin de nettoyage",
    };
    expect(devisSchema.safeParse(validData).success).toBe(true);
  ...
  it("rejette les numéros de téléphone dépassant 30 caractères", () => {
    const validData = {
      prenom: "Jean",
      telephone: "0612345678",
      besoin: "Besoin de nettoyage",
    ...
    const invalidData = {
      ...validData,
      telephone: "0612345678" + " ".repeat(21),
    };
    const result = devisSchema.safeParse(invalidData);
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  it("rejette les descriptions de besoin dépassant 2000 caractères", () => {
    const validData = {
      prenom: "Jean",
      telephone: "0612345678",
      besoin: "B".repeat(2000),
    };
    expect(devisSchema.safeParse(validData).success).toBe(true);
...
describe("optionsSchema and paiementSchema length constraints", () => {
  it("rejette les options dépassant 100 caractères ou plus de 50 options", () => {
    const optionsSchema = stepSchemas.options;
    expect(optionsSchema.safeParse({ options: ["A".repeat(100)] }).success).toBe(true);

    const longOptionResult = optionsSchema.safeParse({ options: ["A".repeat(101)] });
    expect(longOptionResult.success).toBe(false);

    const tooManyOptionsResult = optionsSchema.safeParse({
      options: Array.from({ length: 51 }, (_, i) => `opt_${i}`),
    });
    expect(tooManyOptionsResult.success).toBe(false);
  });
...
describe("lieuSchema length constraints", () => {
  it("rejette les adresses dépassant 300 caractères", () => {
    ...
    expect(lieuSchema.safeParse(validData).success).toBe(true);
    ...
    const result = lieuSchema.safeParse(invalidData);
    expect(result.success).toBe(false);
    if (!result.success) {
